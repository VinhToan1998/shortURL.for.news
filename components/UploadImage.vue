<template lang="pug">
  v-dialog(:value='value' @input='onClose' max-width='480')
    v-card
      v-card-title Update Image
      v-card-text
        template(v-if='file')
          VueCroppie(ref='croppie' :enableResize='false' :viewport='{ width: 250, height: 250, type: "circle" }' :boundary='{ width: 250, height: 250}' @result='onFinishCropping')
          v-row
            v-col(cols='12' md='6')
              v-btn(@click='onClose' color='error' block) Cancel
            v-col(cols='12' md='6')
              v-btn(@click='onFinishCropping' color='success' block) OK
        template(v-else)
          v-file-input(label='File' @change='onFileSelect' v-model='file' outlined dense messages='Select file to continue' accept='image/*')             
</template>
